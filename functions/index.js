...
// Data judgement and storage intent
app.intent('is_data', (conv) => {
	//split user conversation to real data.	
	let words = conv.query.split(' ');
	let obj = words[2];
	let locat = words[words.length-1];
	let cate = obj + "'s location";
	var dataObj = {
		"category": obj,
		"location": locat,
		"type": "data"
	}
	db.collection(FirestoreNames.TIPS).doc(cate).set(dataObj)
	  .then(function() {
...
// Find specific Object and return it's location.
app.intent('get_data', (conv) => {
	//console.log("++++++++++++++++");
	let words = conv.query.split(' ');
	let obj = words[words.length-1];
	var locat = '';
	return db.collection(FirestoreNames.TIPS).doc(obj + '\'s location')
		.get()
		.then(doc => {
			if (!doc.exists) {
			  console.log('No such object!');
			} else {
			  //console.log('Object location:', doc.data().location);
			  locat = doc.data().location;
			  //console.log('@@@@@@@@@' + locat);
			  conv.ask(obj + '\'s location is: ' + locat);			  
			}
		 })
		 .catch(err => {
			console.log('Error finding object', err);
		 });
});
...
app.intent('tell_tip', (conv, params) => {
  ...
  if(category === ADD_DATA) {
	  conv.ask("Please input your data");  
  }
  if(category === FIND_DATA) {
	  conv.ask("Please input your find object");  
  }
...
app.intent('Default Welcome Intent', (conv) => {
  ...
  var tipsRef = db.collection(FirestoreNames.TIPS);
  return tipsRef.where(FirestoreNames.TYPE, '==', 'show')
    .get()
    .then((querySnapshot) => {
